...
const deleteUser = async (req, res) => {
  try {
    ...
    // If the user has a store, we should delete the store as well (which cascades down)
    const user = await prisma.user.findUnique({
      where: { id: userId },
      include: { store: true },
    });
    ...
    // Now delete the user (will cascade delete their feedbacks, ratings, orders, cart items)
...
const deleteStore = async (req, res) => {
  try {
    ...
    // Deleting the store will cascade to its products, ratings, and orders
